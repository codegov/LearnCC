...
namespace leveldb
{
    ...
    static const int kTargetFileSize = 2 * 1048576;   // 2*1048576=2*1024*1024   2M
    ...
    static double MaxBytesForLevel(int level)
    {
        ...
        double result = 10 * 1048576.0;  // Result for both level-0 and level-1  // 10M
        while (level > 1)
        {
            result *= 10;
            level--;
    ...
    bool SomeFileOverlapsRange(const InternalKeyComparator& icmp, bool disjoint_sorted_files, const std::vector<FileMetaData*>& files, const Slice* smallest_user_key, const Slice* largest_user_key)
    {
        ...
        if (!disjoint_sorted_files) // 有交集的排序文件，即可能一个key，在多个文件处都存在。这样就不能使用二分查找key属于哪个文件。
        {
    ...
    {
    public:
        LevelFileNumIterator(const InternalKeyComparator& icmp, const std::vector<FileMetaData*>* flist): icmp_(icmp), flist_(flist), index_(flist->size())
        {        // Marks as invalid
        }
    ...
    static Iterator* GetFileIterator(void* arg, const ReadOptions& options, const Slice& file_value)
    {
        ...
        if (file_value.size() != 16) // number和file_size各占8个字节
        {
    ...
    static void SaveValue(void* arg, const Slice& ikey, const Slice& v)
    {
        ...
        } else
        {
            if (s->ucmp->Compare(parsed_key.user_key, s->user_key) == 0)
            {
                ...
                if (s->state == kFound)
                {
                    s->value->assign(v.data(), v.size()); // 赋值
                }
            }
        }
    }
    ...
    void Version::ForEachOverlapping(Slice user_key, Slice internal_key, void* arg, bool (*func)(void*, int, FileMetaData*))
    {
        ...
        if (!tmp.empty())
        {
            ...
            {
                if (!(*func)(arg, 0, tmp[i])) // 如果key存在于2个以上文件中，则不继续匹配
                {
                    return;
                }
            }
        ...
        for (int level = 1; level < config::kNumLevels; level++)
        {
            ...
            if (index < num_files)
            {
                ...
                {
                    if (!(*func)(arg, level, f)) // 如果key存在于2个以上文件中，则不继续匹配
                    {
                        return;
                    }
                }
    ...
    bool Version::RecordReadSample(Slice internal_key)
    {
        ...
        struct State
        {
            ...
            static bool Match(void* arg, int level, FileMetaData* f)
            {
                ...
                if (state->matches == 1)
                {
                    // Remember first match.  记录key存在的第一个文件
                    state->stats.seek_file = f;
                    state->stats.seek_file_level = level;
                }
                // We can stop iterating once we have a second match.
                return state->matches < 2; // 如果key在第二个文件也存在，则不继续匹配
            }
        };
        
        State state;
        state.matches = 0;
        ForEachOverlapping(ikey.user_key, internal_key, &state, &State::Match); // 查看key是否存在于两个以上文件中
    ...
    void Version::GetOverlappingInputs(int level, const InternalKey* begin, const InternalKey* end, std::vector<FileMetaData*>* inputs)
    {
        ...
        for (size_t i = 0; i < files_[level].size(); )
        {
            ...
            } else
            {
                ...
                /**
                 需要对level0进行特殊处理：
                 例如在level0的第x个文件中，如果file_start和file_limit与user_begin和user_end是
                 区间[start, begin, end, limit] 或 [start, begin, limit, end] 或 [begin, start, end, limit] 的关系。
                 由于level0，文件之间可能存在重叠，所以文件x的区间[start, beign]或者[end, limit]可能与level0其他文件重叠。
                 所以需要扩大[begin, end]的区间。
                 */
                if (level == 0)
                {
        ...
        struct BySmallestKey
        {
            ...
            bool operator()(FileMetaData* f1, FileMetaData* f2) const
            {
                ...
                if (r != 0) {
                    ...
                    return (f1->number < f2->number); // 升序
                }
            }
        };
        
        typedef std::set<FileMetaData*, BySmallestKey> FileSet; //set自定义排序,FileMetaData*是元素，BySmallestKey是排序规则
        struct LevelState
        {
    ...
    public:
        ...
        void Apply(VersionEdit* edit)
        {
            ...
            for (size_t i = 0; i < edit->new_files_.size(); i++)
            {
                ...
                f->allowed_seeks = (f->file_size / 16384);   // 16384==16*1024;  16KB
                if (f->allowed_seeks < 100) f->allowed_seeks = 100;
                
                levels_[level].deleted_files.erase(f->number); // 移除删除元素
        ...
        void SaveTo(Version* v)
        {
            ...
            for (int level = 0; level < config::kNumLevels; level++)
            {
                ...
                for (FileSet::const_iterator added_iter = added->begin(); added_iter != added->end(); ++added_iter)
                {
                    ...
                    // 对std::vector进行自定义排序插入操作
                    // 自定义比较器cmp， 需要将*added_iter插入到base_iter和base_end之间。
                    // 则先遍历base_iter和*added_iter之间文件，添加到容器中。再添加*added_iter文件。
                    // 遍历added下一个文件时，因为added里的文件是排序好的，所以下一个要插入的文件肯定在*added_iter和base_end之间。
    ...
    : env_(options->env), dbname_(dbname), options_(options), table_cache_(table_cache), icmp_(*cmp), next_file_number_(2), manifest_file_number_(0),  // Filled by Recover()
    last_sequence_(0), log_number_(0), prev_log_number_(0), descriptor_file_(NULL), descriptor_log_(NULL), dummy_versions_(this), current_(NULL)
    {
    ...
    Status VersionSet::LogAndApply(VersionEdit* edit, port::Mutex* mu)
    {
        ...
        // Unlock during expensive MANIFEST log write
        {
            ...
            {
                s = SetCurrentFile(env_, dbname_, manifest_file_number_); // current文件用来记载当前的manifest文件名
            }
    ...
    // in *smallest, *largest.
    // REQUIRES: inputs is not empty
    void VersionSet::GetRange2(const std::vector<FileMetaData*>& inputs1, const std::vector<FileMetaData*>& inputs2, InternalKey* smallest, InternalKey* largest)
    {
    ...
    void VersionSet::SetupOtherInputs(Compaction* c)
    {
        ...
        if (false)
        {
            Log(options_->info_log, "Compacting %d '%s' .. '%s'", level, smallest.DebugString().c_str(), largest.DebugString().c_str());
        }
    ...
    input_version_(NULL), grandparent_index_(0), seen_key_(false), overlapped_bytes_(0)
    {
