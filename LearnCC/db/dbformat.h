...
namespace leveldb
{
    ...
    namespace config
    {
        ...
        static const int kReadBytesPeriod = 1048576; // 2^20
    ...
    {
        return key.user_key.size() + 8; // 这里的8个字节(64位)是由sequence(56位)和type(8位)组成
    }
    ...
    {
    private:
        std::string rep_;// 格式是Key+SequenceNumber+ValueType
    public:
    ...
    {
        const size_t n = internal_key.size();
        if (n < 8) return false; // 后8个字节，存储sequence和type
        uint64_t num = DecodeFixed64(internal_key.data() + n - 8);
        unsigned char c = num & 0xff; // unsigned char大小是一个字节，获得低8位，存储的是type
        result->sequence = num >> 8;  // 向右移动8位，获得高56位，存储的是sequence
    ...
    private:
        /**
         LookupKey的结构是klength+userkey+sequence+type
         */
        // We construct a char array of the form:
        //    klength  varint32               <-- start_   // klength=userkey.length+8的可变长度编码,<=4个字节
