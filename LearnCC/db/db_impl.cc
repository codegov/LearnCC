...
namespace leveldb
{
    ...
    struct DBImpl::CompactionState
    {
        ...
        /**
         explicit构造函数是用来防止隐式转换的。请看下面的例子：
    ...
    Status DBImpl::RecoverLogFile(uint64_t log_number, VersionEdit* edit, SequenceNumber* max_sequence)
    {
        struct LogReporter : public log::Reader::Reporter
        {
            ...
            {
                Log(info_log, "%s%s: dropping %d bytes; %s", (this->status == NULL ? "(ignoring error) " : ""), fname, static_cast<int>(bytes), s.ToString().c_str());
    ...
    Iterator* DBImpl::NewIterator(const ReadOptions& options)
    {
        ...
        return NewDBIterator(this, user_comparator(), iter, (options.snapshot != NULL ? reinterpret_cast<const SnapshotImpl*>(options.snapshot)->number_ : latest_snapshot), seed);
    }
