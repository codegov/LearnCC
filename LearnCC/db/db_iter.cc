...
namespace leveldb
{
    ...
    namespace
    {
        ...
        public:
            ...
            : db_(db), user_comparator_(cmp), iter_(iter), sequence_(s), direction_(kForward), valid_(false), rnd_(seed), bytes_counter_(RandomPeriod())
            {
            }
                ...
                if (ParseKey(&ikey) && ikey.sequence <= sequence_)
                {
                    switch (ikey.type)
                    {
                        ...
                        case kTypeValue:
                            ...
                            {
                                // 找到，清空saved_key并返回，iter_已定位到正确的entry
                                valid_ = true;
                                saved_key_.clear();
                                return;
                            }
